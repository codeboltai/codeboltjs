...
export async function startJob(
    ...
    try {
        console.log(`[JobExecutor] Starting job: ${job.name} (${job.jobId})`);
        console.log(`[JobExecutor] Using worker agent: ${ctx.workerAgentId}`);
        codebolt.chat.sendMessage(`Starting job: ${job.name} (worker: ${ctx.workerAgentId})`);

        // Validate worker agent ID
        if (!ctx.workerAgentId) {
            const errorMsg = 'No worker agent ID configured. Please set defaultWorkerAgentId in orchestrator config.';
        ...
        if (response && response.success === false) {
            ...
            console.error(`[JobExecutor] Failed to start job: ${job.name}`, errorMsg);
        ...
        if (response.threadId) {
            console.log(`[JobExecutor] Job started: ${job.name} (threadId: ${response.threadId})`);
        ...
        console.error(`[JobExecutor] Error starting job: ${job.name}`, error);
...
export async function handleJobCompletion(
    ...
    console.log(`[JobExecutor] Job ${success ? 'completed' : 'failed'}: ${jobId}`);
    ...
    for (const { job, result } of results) {
        if (result.success) {
            ...
            if (result.threadId) {
                ...
                console.log(`[JobExecutor] Mapped threadId ${result.threadId} to jobId ${job.jobId}`);
            }
        } else {
...
export async function executeJobsWithDependencies(
    ...
    console.log(`[JobExecutor] Initial jobs to start: ${initialJobs.length} (${initialType})`);
    ...
    console.log(`[JobExecutor] Started ${started.length} jobs, ${failed.length} failed to start`);
        ...
        try {
            ...
            if (event.type === 'backgroundAgentCompletion' ||
                ...
                if (jobId && tracker.getJob(jobId)) {
                    ...
                    console.warn(`[JobExecutor] Could not find jobId in completion event or job not tracked`);
                    console.warn(`[JobExecutor] Event data:`, JSON.stringify(completionData, null, 2));
                }
            }
        } catch (error) {
            console.error('[JobExecutor] Error waiting for event:', error);
            // Continue the loop
        }
    }
