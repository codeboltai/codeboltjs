...
let currentHandle: ReturnType<typeof ThirdPartyAgents.opencode> | null = null;
...
codebolt.onMessage(async (userMessage: any) => {
    try {
        ...
        if (!messageContent.trim()) {
            console.log('[opencode-thirdparty] Empty message, skipping');
            return;
        }

        const trimmed = messageContent.trim();
        console.log(`[opencode-thirdparty] Message: "${trimmed.substring(0, 100)}"`);

        if (currentHandle && currentHandle.state === 'running') {
            console.log('[opencode-thirdparty] Agent running — sending to stdin');
        ...
        currentHandle = ThirdPartyAgents.opencode(trimmed, {
            codebolt,
        ...
        currentHandle = null;
        console.error(`[opencode-thirdparty] Error: ${error}`);
...
console.log('[opencode-thirdparty] Agent ready. Listening for messages.');
