...
export const dispatchJsonlEntry = (entry: any) => {
    match(entry)
            ...
            if (typeof content === 'string') {
                ...
                for (const block of content) {
                    match(block)
                        ...
                        .otherwise(() => {
                            // images, documents - send as generic
        ...
        .with({ type: 'assistant' }, (e: any) => {
            ...
            for (const block of content) {
                match(block)
                    ...
                    // Tool use - dispatch based on tool name
        ...
        .with({ type: 'progress' }, (e: any) => {
            // Progress indicators (e.g. tool execution progress)
            if (e.data) {
        ...
        .with({ type: 'file-history-snapshot' }, () => {
            // File state tracking, informational
        })
...
/**
 * Dispatch a tool_use content block from an assistant message.
 */
...
/**
 * Dispatch a tool_result content block from a user message.
 * These contain the results of tool executions.
 */
function dispatchToolResult(result: any) {
    ...
    // We don't know which tool this result is for from the result alone,
    // so send as a generic response. The UI can correlate via tool_use_id.
    codebolt.notify.chat.AgentTextResponseNotify(
