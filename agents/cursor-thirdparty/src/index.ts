...
let currentHandle: ReturnType<typeof ThirdPartyAgents.cursor> | null = null;
...
codebolt.onMessage(async (userMessage: any) => {
    try {
        ...
        if (!messageContent.trim()) {
            console.log('[cursor-thirdparty] Empty message, skipping');
            return;
        }

        const trimmed = messageContent.trim();
        console.log(`[cursor-thirdparty] Message: "${trimmed.substring(0, 100)}"`);

        if (currentHandle && currentHandle.state === 'running') {
            console.log('[cursor-thirdparty] Agent running — sending to stdin');
        ...
        // Parse mode from /plan or /ask commands
        let prompt = trimmed;
        ...
        currentHandle = ThirdPartyAgents.cursor(prompt, {
            codebolt,
        ...
        currentHandle = null;
        console.error(`[cursor-thirdparty] Error: ${error}`);
...
console.log('[cursor-thirdparty] Agent ready. Listening for messages.');
