...
const PLUGIN_SDK_FEATURE_MODULES = {
  ...
  panelUi: ['package.json#codebolt.plugin.ui.path', 'ui/default/index.html', 'relative assets'],
  panelOpen: ['plugin.dynamicPanel.open', 'plugin.dynamicPanel.update', 'plugin.dynamicPanel.close', 'plugin.dynamicPanel.list'],
  panelMessaging: ['plugin.dynamicPanel.onMessage', 'plugin.dynamicPanel.offMessage', 'plugin.dynamicPanel.send'],
  panelRouter: ['plugin.dynamicPanel.router', 'router.get', 'router.post', 'router.put', 'router.patch', 'router.delete', 'router.all', 'router.emit', 'router.dispose'],
  toolBackedPanel: ['pluginTools.registerTool', 'pluginTools.registerTools'],
  statefulPanel: ['plugin.kvStore', 'plugin.notify', 'plugin.artifact'],
};
...
const PLUGIN_SDK_API_CATALOG = [
  {
    ...
    calls: ['import plugin from "@codebolt/plugin-sdk"', 'import type { DynamicPanelRouterContext } from "@codebolt/plugin-sdk"'],
  },
  {
    module: 'plugin lifecycle',
    purpose: 'Start panel bridge handlers and cleanup resources.',
    calls: ['plugin.onStart(async (context) => { ... })', 'plugin.onStop(async () => { ... })', 'await plugin.waitForReady()', 'plugin.pluginId', 'plugin.pluginDir'],
  },
  {
    module: 'UI metadata',
    purpose: 'Tell pluginRoutes where to serve dynamic panel UI assets.',
    calls: ['package.json codebolt.plugin.ui.path: ui/default', 'ui/default/index.html', 'relative CSS and JS asset paths'],
  },
  {
    module: 'dynamicPanel messaging',
    purpose: 'Exchange messages between iframe UI and plugin process.',
    calls: ['plugin.dynamicPanel.open(panelId, title, html, options)', 'plugin.dynamicPanel.update(panelId, html)', 'plugin.dynamicPanel.send(panelId, payload)', 'plugin.dynamicPanel.onMessage(panelId, handler)', 'plugin.dynamicPanel.offMessage(panelId)', 'plugin.dynamicPanel.close(panelId)', 'plugin.dynamicPanel.list()'],
  },
  {
    module: 'dynamicPanel router',
    purpose: 'Expose fetch-style panel endpoints for richer UI behavior.',
    calls: ['const router = plugin.dynamicPanel.router(panelId)', 'router.get(path, handler)', 'router.post(path, handler)', 'router.use(handler)', 'router.emit(event, payload)', 'router.dispose()'],
  },
  {
    module: 'panel frontend messaging',
    purpose: 'Use iframe postMessage to reach the plugin bridge.',
    calls: ['window.parent.postMessage({ type, data }, "*")', 'window.addEventListener("message", handler)'],
  },
];

const PLUGIN_SDK_USAGE_RULES = [
  'Use default import for plugin lifecycle and dynamicPanel facade calls.',
  'Set codebolt.plugin.ui.path and include a real ui/default/index.html file.',
  'Implement at least one panel-to-plugin message path and one plugin-to-panel response path.',
  'Use router only when route-like request handling is useful; simple panels can use onMessage and send.',
...
const PLATFORM_CONTRACT = {
  ...
  artifactShape: [
    ...
    '<name>/src/index.ts',
    '<name>/dist/index.js',
    '<name>/ui/default/index.html',
    '<name>/README.md',
  ],
  manifestShape: [
    'package.json: main: dist/index.js, scripts.build, dependencies, codebolt.plugin metadata.',
    'codebolt.plugin.ui.path: ui/default or the generated dynamic panel UI directory.',
  ],
  sourceShape: [
    'src/index.ts imports plugin APIs from @codebolt/plugin-sdk.',
    'src/index.ts registers plugin.dynamicPanel.onMessage and uses plugin.dynamicPanel.send for responses/updates.',
    'ui/default/index.html posts panel messages and handles plugin responses using relative assets.',
  ],
  exampleSnippets: [
    'package.json example shape: main: dist/index.js, codebolt.plugin.ui.path: ui/default.',
    'src/index.ts example shape: import plugin from "@codebolt/plugin-sdk"; plugin.dynamicPanel.onMessage("main-panel", async (message) => { await plugin.dynamicPanel.send("main-panel", { type: "response", payload: message }); });',
    'ui/default/index.html example shape: panel UI posts typed messages and renders plugin responses.',
  ],
  manifestExpectations: [
    'package.json main is dist/index.js and includes codebolt.plugin metadata.',
    'codebolt.plugin.ui.path points to the generated ui/default directory or equivalent panel UI root.',
    'scripts.build compiles TypeScript from src/index.ts into dist/index.js.',
  ],
  runtimeFlow: [
    'Register plugin.dynamicPanel.onMessage in the plugin process.',
    'Use plugin.dynamicPanel.send for plugin-to-panel updates when needed.',
    'Build UI code that posts messages to the plugin bridge and handles responses.',
    'Keep panel state and message names explicit so parent agents can reason about the UI behavior.',
  ],
  publishSafetyRules: [
    ...
    'Keep panel plugin and UI assets self-contained and avoid external network assumptions unless requested.',
  ],
  generationChecklist: [
    'Create a plugin package with dynamic panel UI and plugin bridge handlers.',
    'Generate package metadata, TypeScript source, build config, runtime dist output, UI HTML, and README.',
    'Implement at least one clear panel-to-plugin message path and one plugin-to-panel response path.',
    'Document message contracts, UI path, startup behavior, and configuration.',
  ],
  verificationChecklist: [
    'All required files exist, including ui/default/index.html.',
    'dynamicPanel.onMessage and dynamicPanel.send appear in generated files.',
    'package.json has codebolt.plugin.ui.path, dist/index.js as main, and a build script.',
    'No local dependency specifiers or local filesystem references are present.',
    'dist/index.js has valid JavaScript syntax.',
  ],
  commonFailureModes: [
    'Creating static UI without plugin.dynamicPanel handlers.',
    'Forgetting codebolt.plugin.ui.path.',
    'Using absolute asset paths that break after publishing.',
...
async function runActionBlockMiniAgent(threadContext) {
  ...
  try {
    ...
    while (!verification.passed && repairPassCount < agentLoop.maxRepairPasses) {
      repairPassCount += 1;
      await codebolt.chat.sendMessage('[dynamic-panel] mini-agent repair loop started', {
