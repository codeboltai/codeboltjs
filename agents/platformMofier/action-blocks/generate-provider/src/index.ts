...
const CODEBOLT_FEATURE_MODULES = {
  providerEntrypoint: ['onProviderStart', 'onProviderAgentStart', 'onProviderStop', 'environment'],
...
const CODEBOLT_API_USAGE_RULES = [
  ...
  'For provider runtime work, document external service credentials, startup checks, health checks, and cleanup behavior.',
  ...
  'For dynamic UI, use dynamicPanel only when the provider needs an agent-visible panel; plugin-owned panels require plugin SDK metadata, UI assets, and event handlers.',
...
const PLATFORM_CONTRACT = {
  ...
  npmDependencies: ['@codebolt/codeboltjs:^5.1.36', '@codebolt/types:^5.1.12', 'typescript:^5.4.5'],
...
async function runActionBlockMiniAgent(threadContext) {
  ...
  try {
    ...
    while (!verification.passed && repairPassCount < agentLoop.maxRepairPasses) {
      repairPassCount += 1;
      await codebolt.chat.sendMessage('[provider] mini-agent repair loop started', {
