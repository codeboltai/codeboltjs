...
const PLATFORM_CONTRACT = {
  ...
  manifestExpectations: [
    ...
    'The plugin metadata identifies the web search provider and any login/configuration needs.',
...
async function runAgentLoopForSpec(spec, repairErrors) {
  ...
  const reqMessage = {
    ...
    messageId: `platform-mofier-websearch-plugin-${Date.now()}`,
    threadId: process.env.THREAD_ID || `platform-mofier-websearch-plugin-${Date.now()}`,
    mentionedAgents: [],
    mentionedFiles: [],
...
async function runActionBlockMiniAgent(threadContext) {
  ...
  try {
    await codebolt.chat.sendMessage('[websearch-plugin] mini-agent loop started', {
    ...
    if (!verification.passed) {
      await codebolt.chat.sendMessage('[websearch-plugin] mini-agent repair loop started', verification);
