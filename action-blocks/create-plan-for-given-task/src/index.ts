...
const DETAIL_PLANNER_SYSTEM_PROMPT = `You are Codebolt CLI, an expert AI assistant operating in a special 'Plan Mode'. Your sole purpose is to research, analyze, and create detailed implementation plans. You must operate in a strict read-only capacity.
...
const TASK_PLANNER_SYSTEM_PROMPT = `Here is the detailed plan:
\`\`\`
{{PLAN_CONTENT}}
\`\`\`
...
codebolt.onActionBlockInvocation(async (threadContext, _metadata): Promise<PlanResult> => {
    try {
        ...
        // Get user message from thread context
        const userMessage = threadContext?.userMessage as FlatUserMessage;
