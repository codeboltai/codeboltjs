...
/**
 * find-next-job-for-agent Action Block
 *
 * Finds and assigns the next available job to an agent using pheromone-based selection.
 *
 * Process:
 ...
 *    - Normal jobs with resolved dependencies
 *    - Blocked jobs with mightbecompleted pheromone
 *    - Splittable jobs
 *    - Last resort blocked jobs
 ...
 *   - swarmConfig: (optional) Configuration for job splitting
 *
 * Output:
 *   - success: boolean
 *   - job: { id, name, description, status, groupId } (if found)
 *   - action: 'implement' | 'split' | 'terminate' | 'free-request' | null
...
codebolt.onActionBlockInvocation(async (threadContext, metadata): Promise<FindJobResult> => {
    try {
        ...
        const ctx: AgentContext = {
            ...
            swarmName: vars.swarmName || 'Default Swarm',
            agentId: vars.agentId || metadata.parentAgentInstanceId || '',
            agentName: vars.agentName || `Agent:${metadata.parentAgentInstanceId}`,
            capabilities: vars.capabilities || ['coding'],
            requirements: vars.requirements || ''
        };

        // Default swarm config if not provided
        const swarmConfig: SwarmConfig = vars.swarmConfig || {
