...
/**
 * Notification Event Schemas for Agent-to-App Communication
 * Based on notificationMessageSchemas.ts
 */

// Re-defining necessary enums and base schemas to be self-contained or compatible
...
// Info with link schema (ported from coreMessageSchemas)
export const infoWithLinkSchema = baseMessageSchema.extend({
    type: z.literal('message'),
    templateType: z.literal('INFOWITHLINK'),
    content: z.string(),
    data: z.object({
        text: z.string(),
        linkUrl: z.enum(['Memory', 'Editor', 'Terminal', 'Browser', 'Preview', 'Git', 'Debug']).optional(), // ComponentType
    }),
});
