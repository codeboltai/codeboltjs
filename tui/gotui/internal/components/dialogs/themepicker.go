...
// View renders the picker overlay as a dialog.
func (p *ThemePicker) View(width, height int) string {
	panel, ok := p.dialogPanel(width)
	if !ok || height <= 0 {
		return ""
	}
	return Wrap(panel, width, height)
}

// Layer renders the picker as an overlay layer.
func (p *ThemePicker) Layer(width, height int) *lipgloss.Layer {
	panel, ok := p.dialogPanel(width)
	if !ok || height <= 0 {
		return nil
	}
...
func (p *ThemePicker) dialogPanel(width int) (string, bool) {
