 ...
 *
 * Builds a sandbox template with codebolt cli pre-installed from npm.
 *
...
async function main() {
  console.log(
    `Building E2B template (node:22-trixie + codebolt@${CLI_VERSION})...\n`,
  );
  ...
  const buildInfo = await Template.build(template, 'codebolt-remote-template', {
    cpuCount: 2,
