...
export class CodeboltWebSocketClient {
  ...
  public async initializeWebSocket(): Promise<void> {
    const wsUrl = `ws://${this.config.serverUrl}:${this.config.socketPort}/codebolt?agentId=${this.config.agentId}&IS_PROVIDER=true&IS_REMOTE_PROVIDER=true&providerId=${this.config.providerId}&storedTaskId=${this.config.storedTaskId}&environmentId=${this.config.environmentId}`;
