...
import { executeGrepSearch, type GrepMatch } from '../utils/search/GrepSearch';
...
> {
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    ...
    // Create a temporary instance of GrepTool to access the resolveAndValidatePath method
    const tempGrepTool = new GrepTool(this.config);
    // Use reflection to access the private method
    const searchDirAbs = (tempGrepTool as any).resolveAndValidatePath(this.params.path);
...
`;
  ...
  private resolveAndValidatePath(relativePath?: string): string | null {
    ...
    const targetPath = path.resolve(this.config.targetDir, relativePath);
