...
/**
 * Configuration interface for SearchFileContent
 */
export interface SearchFileContentConfig {
  /** Workspace context */
...
/**
 * Result interface for file content search
 */
...
/**
 * Parameters for file content search
 */
export interface SearchFileContentParams {
  pattern: string;
  path?: string;
  include?: string;
}
...
/**
 * Service for searching file content with exact logic from GrepTool
 */
export class SearchFileContent {
  private config: SearchFileContentConfig;

  constructor(config: SearchFileContentConfig) {
    this.config = config;
  }
   ...
   */
  private getGlobExcludes(): string[] {
    return [
      'node_modules',
      '.git',
      'dist',
  ...
  /**
   * Parses the standard output of grep-like commands.
   */
  ...
  private async performGrepSearch(options: {
    ...
    try {
      // --- Strategy 1: git grep ---
      const isGit = this.isGitRepository(absolutePath);
      ...
      if (gitAvailable) {
        ...
        try {
          ...
          console.debug(
            `SearchFileContent: git grep failed: ${getErrorMessage(
              gitError,
            )}. Falling back...`,
          );
        }
      ...
      if (grepAvailable) {
        ...
        const globExcludes = this.getGlobExcludes();
        ...
        try {
          ...
          console.debug(
            `SearchFileContent: System grep failed: ${getErrorMessage(
              grepError,
            )}. Falling back...`,
          );
        }
      ...
      console.debug(
        'SearchFileContent: Falling back to JavaScript grep implementation.',
      );
      strategyUsed = 'javascript fallback';
      const globPattern = include ? include : '**/*';
      const ignorePatterns = this.getGlobExcludes().map(p => `**/${p}/**`);
      ...
      for await (const filePath of filesStream) {
        ...
        try {
          ...
          if (!isNodeError(readError) || readError.code !== 'ENOENT') {
            console.debug(
              `SearchFileContent: Could not read/process ${fileAbsolutePath}: ${getErrorMessage(
                readError,
              )}`,
            );
          }
      ...
      console.error(
        `SearchFileContent: Error in performGrepSearch (Strategy: ${strategyUsed}): ${getErrorMessage(
          error,
        )}`,
      );
  ...
  /**
   * Search for content within files with exact logic from GrepTool
   */
  async searchFileContent(params: SearchFileContentParams, signal?: AbortSignal): Promise<SearchFileContentResult> {
    try {
      const workspaceDirectories = this.config.workspaceContext.getDirectories();
      ...
      for (const searchDir of searchDirectories) {
        ...
        if (params.include !== undefined) {
          searchOptions.include = params.include;
        }
        const matches = await this.performGrepSearch(searchOptions);
      ...
      console.error(`Error during SearchFileContent execution: ${error}`);
      const errorMessage = getErrorMessage(error);
      return {
        success: false,
        error: `Error during grep search operation: ${errorMessage}`,
      };
    }
  }

  /**
   * Validate parameters for file content search
   */
  validateParams(params: SearchFileContentParams): string | null {
    try {
...
/**
 * Create a SearchFileContent instance
 */
export function createSearchFileContent(config: SearchFileContentConfig): SearchFileContent {
  return new SearchFileContent(config);
}