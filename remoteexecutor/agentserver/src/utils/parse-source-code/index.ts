...
// Try to import globby, fallback to null if not available
let globby: any = null;
try {
  const globbyModule = require('@codebolt/globby');
  globby = globbyModule.globby;
} catch (error) {
  console.warn('parse-source-code: @codebolt/globby not available, will use fs fallback');
}
...
export async function globbyLevelByLevel(options) {
	let results = []
...
export function separateFiles(allFiles) {
	const extensions = [
		"js",
		"jsx",
		"ts",
...
*/
export async function parseFile(filePath, languageParsers) {
