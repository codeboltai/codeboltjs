import { BaseAcknowledgeNode } from '@codebolt/agent-shared-nodes';
import codebolt from '@codebolt/codeboltjs';
import { handleMailResponse } from './utils';

export class AcknowledgeNode extends BaseAcknowledgeNode {
    async onExecute() {
        const messageId = this.getInputData(1) as string;
        const agentId = this.getInputData(2) as string;

        try {
            const response = await codebolt.mail.acknowledge({ messageId, agentId });
