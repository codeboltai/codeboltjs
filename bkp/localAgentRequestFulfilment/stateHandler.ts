...
export class StateHandler {
  ...
  handleStateEvent(agent: ClientConnection, stateEvent: StateEvent): void {
    ...
    logger.info(formatLogMessage('info', 'StateHandler', `Handling state event: ${action} from ${agent.id}`));
    ...
    logger.info(formatLogMessage('info', 'StateHandler', `Sent state request notification: ${action}`));
