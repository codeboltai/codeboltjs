...
/**
 * Dispatcher for OpenAI Codex CLI messages.
 *
 * Codex executes tools internally, so there are no tool_use dispatches.
...
export class CodexDispatcher extends BaseDispatcher {
    ...
    dispatch(message: CodeboltMessage, codebolt: CodeboltInstance): void {
        switch (message.type) {
            case 'init':
                console.log(`[codex-dispatcher] init (sessionId=${message.sessionId})`);
            ...
            case 'assistant_text':
                if (message.text) {
                    console.log(`[codex-dispatcher] assistant_text (${message.text.length} chars)`);
            ...
            case 'result':
                console.log(`[codex-dispatcher] result (cost=$${message.usage?.costUsd?.toFixed(4) ?? '?'})`);
                if (this._currentStreamId) {
            ...
            case 'error':
                console.log(`[codex-dispatcher] error: ${(message.text || '').substring(0, 80)}`);
                if (message.text) {
