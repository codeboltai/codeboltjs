...
/**
 * Dispatcher for Gemini CLI messages.
 *
 * Gemini executes tools internally, so there are no tool_use dispatches.
...
export class GeminiDispatcher extends BaseDispatcher {
    ...
    dispatch(message: CodeboltMessage, codebolt: CodeboltInstance): void {
        switch (message.type) {
            case 'init':
                console.log(`[gemini-dispatcher] init (model=${message.model}, sessionId=${message.sessionId})`);
            ...
            case 'assistant_text':
                if (message.text) {
                    console.log(`[gemini-dispatcher] assistant_text (${message.text.length} chars)`);
            ...
            case 'result':
                console.log(`[gemini-dispatcher] result (cost=$${message.usage?.costUsd?.toFixed(4) ?? '?'})`);
                if (this._currentStreamId) {
            ...
            case 'error':
                console.log(`[gemini-dispatcher] error: ${(message.text || '').substring(0, 80)}`);
                if (message.text) {
