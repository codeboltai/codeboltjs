...
/**
 * Dispatcher for Cursor Agent CLI messages.
 *
 * Cursor executes tools internally, so there are no tool_use dispatches.
...
export class CursorDispatcher extends BaseDispatcher {
    ...
    dispatch(message: CodeboltMessage, codebolt: CodeboltInstance): void {
        switch (message.type) {
            case 'init':
                console.log(`[cursor-dispatcher] init (model=${message.model}, sessionId=${message.sessionId})`);
            ...
            case 'assistant_text':
                if (message.text) {
                    console.log(`[cursor-dispatcher] assistant_text (${message.text.length} chars)`);
            ...
            case 'result':
                console.log(`[cursor-dispatcher] result (cost=$${message.usage?.costUsd?.toFixed(4) ?? '?'})`);
                if (this._currentStreamId) {
            ...
            case 'error':
                console.log(`[cursor-dispatcher] error: ${(message.text || '').substring(0, 80)}`);
                if (message.text) {
