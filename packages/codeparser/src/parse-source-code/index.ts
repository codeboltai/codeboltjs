...
// Use require for globby as it might have compatibility issues with ES modules
const { globby } = require('@codebolt/globby');
export const LIST_FILES_LIMIT = 200

// TODO: implement caching behavior to avoid having to keep analyzing project for new tasks.
export async function parseSourceCodeForDefinitionsTopLevel(dirPath:string) {
...
export async function globbyLevelByLevel(options: any): Promise<string[]> {
	let results: string[] = []
...
export function separateFiles(allFiles: string[]): { filesToParse: string[], remainingFiles: string[] } {
	const extensions = [
		"js",
		"jsx",
		"ts",
	...
	const filesToParse = allFiles.filter((file: string) => extensions.includes(path.extname(file))).slice(0, 50) // 50 files max
	const remainingFiles = allFiles.filter((file: string) => !filesToParse.includes(file))
...
*/
export async function parseFile(filePath: string, languageParsers: any): Promise<string | undefined> {
