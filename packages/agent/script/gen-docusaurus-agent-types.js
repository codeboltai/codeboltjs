...
// Configuration
const CONFIG = {
    // Source entry points (multiple modules)
    entryPoints: [
        path.resolve(__dirname, '../src/unified/index.ts'),
        path.resolve(__dirname, '../src/processor-pieces/index.ts'),
    ],
...
function buildTypedocCommand(outDir, watch = false) {
    ...
    args.push('--tsconfig', CONFIG.tsconfig);
...
async function main() {
    ...
    CONFIG.entryPoints.forEach(entry => console.log(`     - ${entry}`));
