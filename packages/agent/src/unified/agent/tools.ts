...
export class Tool implements ToolInterface {
    ...
    private validateInput(input: unknown): { valid: boolean; error?: string } {
        try {
     ...
     */
    private validateOutput(output: unknown): { valid: boolean; error?: string } {
    ...
    private zodTypeToJsonSchema(zodType: z.ZodType): any {
        ...
        if (zodType instanceof z.ZodUnion) {
            ...
            const anyOf = options.map((option: z.ZodType) => this.zodTypeToJsonSchema(option));
        ...
        if (zodType instanceof z.ZodNullable) {
            const innerSchema = this.zodTypeToJsonSchema((zodType as any)._def.innerType);
            return {
                anyOf: [
                    innerSchema,
                    { type: 'null' }
