...
import { InitialPromptGenerator, type UnifiedMessageProcessor } from '../base/initialPromptGenerator';
import { createUnifiedAgentStep } from '../base/agentStep';
import { createUnifiedResponseExecutor } from '../base/responseExecutor';
...
export class UnifiedAgent {
    private messageModifier: UnifiedMessageProcessor;
    private agentStep: UnifiedAgentStep;
    private responseExecutor: UnifiedResponseExecutor;
    ...
    constructor(config: UnifiedAgentConfig & {
        ...
        this.agentStep = createUnifiedAgentStep({
            llmConfig: this.config.llmConfig,
            codebolt: this.config.codebolt,
            enableLogging: this.config.enableLogging
        });

        this.responseExecutor = createUnifiedResponseExecutor({
            codebolt: this.config.codebolt,
    ...
    private setupProcessors(config: {
        ...
        const followUpProcessors: Processor[] = [];
        const preToolCallProcessors: Processor[] = [];

        if (config.defaultProcessors !== false) {
            // Add default follow-up conversation processors
            followUpProcessors.push(
                new ConversationCompactorProcessor({
                    maxConversationLength: this.config.maxConversationLength || 50,
                ...
                new ConversationContinuityProcessor({
